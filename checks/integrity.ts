/**
 * checks/integrity.ts — deterministic prose integrity guard.
 *
 * Ported (near-verbatim) from novel-harness src/lint/integrity.ts. The lint
 * fixer is allowed to change local wording, but it must not create malformed
 * prose. Keep this deterministic and conservative: if a suspicious artifact
 * is newly introduced by the fixed text, reject the whole lint pass and keep
 * the raw draft.
 */
...
function detectFusedBoundaries(text: string): LintFixIntegrityIssue[] {
  const issues: LintFixIntegrityIssue[] = []
  const dottedTokenInternalDots = collectDottedTokenInternalDotOffsets(text)
  for (let i = 0; i < text.length - 1; i++) {
    ...
    if (ch === "." && dottedTokenInternalDots.has(i)) continue
...
// Dotted initials such as O.B. and J.R.R. use internal dots as part of the token.
// The final dot remains a sentence-boundary candidate so O.B.She still fires.
const DOTTED_INITIALS_RE = /\b(?:[A-Z]\.){2,3}/g

function collectDottedTokenInternalDotOffsets(text: string): Set<number> {
  const offsets = new Set<number>()
  for (const match of text.matchAll(ALL_CAPS_DOTTED_RE)) {
    const start = match.index ?? 0
    for (let i = 0; i < match[0].length; i++) {
      if (match[0][i] === ".") offsets.add(start + i)
    }
  }
  for (const match of text.matchAll(DOTTED_INITIALS_RE)) {
    const start = match.index ?? 0
    const token = match[0]
    for (let i = 0; i < token.length - 1; i++) {
      if (token[i] === ".") offsets.add(start + i)
    }
  }
  return offsets
}
...
function normalizeSentence(sentence: string): string {
  ...
  // are NOT treated as duplicate sentences.
  return sentence
    .toLowerCase()
    .replace(/[^a-z0-9' ?!.]+/g, " ")
    .replace(/\s+/g, " ")
 ...
 * beat index (L70b routing shape). Offsets inside the separator are
 * attributed to the LATER beat; negative offsets clamp to beat 0.
 */
