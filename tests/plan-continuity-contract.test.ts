...
describe("validateContinuityContract — complete valid data", () => {
  test("accepts a complete version-1 contract with resolvable fact IDs", () => {
    ...
    expect(validateContinuityContract(plan, KNOWN)).toEqual([])
    const contract = assertContinuityContract(plan, KNOWN)
...
describe("validateContinuityContract — legacy compatibility", () => {
  test("a wholly legacy plan (no contract fields) is readable and unvalidated", () => {
    ...
    expect(validateContinuityContract(plan, KNOWN)).toEqual([])
    expect(assertContinuityContract(plan, KNOWN)).toBeNull()
  })

  test("legacy plans stay readable even with an empty canon", () => {
    expect(validateContinuityContract(legacyPlan(), new Set())).toEqual([])
  })
})
...
describe("validateContinuityContract — unknown fact IDs", () => {
  test("rejects an unknown ID in continuity_anchors.fact_ids", () => {
    ...
    const errors = validateContinuityContract(plan, KNOWN)
  ...
  test("schedule_fact.fact_id is a forward reference (not resolved against canon)", () => {
    ...
    expect(validateContinuityContract(validPlan(), KNOWN)).toEqual([])
  })
})
...
describe("validateContinuityContract — malformed fields", () => {
  test("schedule_fact must be a mapping with nonblank fact_id and text", () => {
    ...
    expect(validateContinuityContract(plan, KNOWN).some(e => e.includes("schedule_fact: must be a mapping"))).toBe(true)

    const plan2 = validPlan()
    plan2.schedule_fact = null as unknown as PlanChapter["schedule_fact"]
    expect(validateContinuityContract(plan2, KNOWN).some(e => e.includes("schedule_fact: must be a mapping"))).toBe(true)

    const plan3 = validPlan()
    plan3.schedule_fact = { fact_id: "  ", text: "ok" }
    expect(validateContinuityContract(plan3, KNOWN).some(e => e.startsWith("schedule_fact.fact_id: must be a nonblank string"))).toBe(true)

    const plan4 = validPlan()
    delete (plan4.schedule_fact as Record<string, unknown>).text
    expect(validateContinuityContract(plan4, KNOWN).some(e => e.startsWith("schedule_fact.text: must be a nonblank string"))).toBe(true)
  })
  ...
  test("continuity_anchors.fact_ids must be an array of nonblank canon IDs", () => {
    ...
    expect(validateContinuityContract(plan, KNOWN).some(e => e.includes("continuity_anchors.fact_ids: must be an array"))).toBe(true)

    const plan2 = validPlan()
    plan2.continuity_anchors.fact_ids = ["fact-1", ""]
    expect(validateContinuityContract(plan2, KNOWN).some(e => e.includes("continuity_anchors.fact_ids[1]: must be a nonblank string"))).toBe(true)

    const plan3 = validPlan()
    delete (plan3.continuity_anchors as Record<string, unknown>).fact_ids
    expect(validateContinuityContract(plan3, KNOWN).some(e => e.includes("continuity_anchors.fact_ids: missing"))).toBe(true)
  })
  ...
  test("character_states entries need nonblank character/location/emotional and string arrays", () => {
    ...
    expect(validateContinuityContract(plan, KNOWN).some(e => e.includes("character_states[0]: must be a mapping"))).toBe(true)

    const plan2 = validPlan()
    plan2.continuity_anchors.character_states = [{ character: "Kael Rusk", location: "Hall" }] as unknown as CharacterStateAnchor[]
    const errors2 = validateContinuityContract(plan2, KNOWN)
    ...
    expect(validateContinuityContract(plan3, KNOWN).some(e => e.includes("character_states[0].knows: must be an array of nonblank strings"))).toBe(true)

    const plan4 = validPlan()
    plan4.continuity_anchors.character_states = [
      { character: "Kael Rusk", location: "Hall", emotional: "wary", knows: [123], does_not_know: [] },
    ] as unknown as CharacterStateAnchor[]
    expect(validateContinuityContract(plan4, KNOWN).some(e => e.includes("character_states[0].knows: must be an array of nonblank strings"))).toBe(true)
  })
  ...
  test("reader_info must be a mapping with both ID arrays present", () => {
    ...
    expect(validateContinuityContract(plan, KNOWN).some(e => e.includes("reader_info: must be a mapping"))).toBe(true)

    const plan2 = validPlan()
    delete (plan2.reader_info as Record<string, unknown>).knows_fact_ids
    expect(validateContinuityContract(plan2, KNOWN).some(e => e.includes("reader_info.knows_fact_ids: missing"))).toBe(true)

    const plan3 = validPlan()
    plan3.reader_info.withhold_fact_ids = "fact-2" as unknown as string[]
    expect(validateContinuityContract(plan3, KNOWN).some(e => e.includes("reader_info.withhold_fact_ids: must be an array"))).toBe(true)
  })
})

describe("required mode (generated plans)", () => {
  test("accepts a complete valid contract and returns it", () => {
    const contract = assertRequiredContinuityContract(validPlan(), KNOWN)
  ...
  test("fails closed on a contractless (legacy) plan", () => {
    expect(() => assertRequiredContinuityContract(legacyPlan(), KNOWN)).toThrow(
  ...
  test("fails closed on a partial contract with useful errors", () => {
    ...
    plan.continuity_contract_version = 1
    plan.schedule_fact = { fact_id: "schedule-ch2", text: "The sale is on the fourteenth day." }
    expect(() => assertRequiredContinuityContract(plan, KNOWN)).toThrow(/invalid continuity contract/)
    expect(() => assertRequiredContinuityContract(plan, KNOWN)).toThrow(/continuity_anchors missing/)
    expect(() => assertRequiredContinuityContract(plan, KNOWN)).toThrow(/reader_info missing/)
  })

  test("fails closed on unknown fact IDs", () => {
    const plan = validPlan()
    plan.continuity_anchors.fact_ids = ["fact-99"]
    expect(() => assertRequiredContinuityContract(plan, KNOWN)).toThrow(/'fact-99' not found in canon\/facts\.md/)
  })
})
...
describe("readPlan — file-level fail-closed behavior", () => {
  ...
  test("existing fixtures stay readable (loop-dry + rillgate ch1 are legacy)", () => {
    const dry = openNovel("tests/fixtures/loop-dry")
    const dryPlan = readPlan(dry, 1)
    expect(dryPlan.chapter).toBe(1)
    expect(hasContinuityContract(dryPlan)).toBe(false)
